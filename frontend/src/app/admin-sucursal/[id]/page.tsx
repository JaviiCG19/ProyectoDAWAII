    ...
    if (!checking && localId) {
      ...
      try {
        ...
        const [dataFranjas, dataPromos] = await Promise.all([
          getFranjasByLocal(idStr),
          getPromocionesByLocal(idStr)
        ]);
        ...
        <section className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8">
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-gray-800">Horario de Local</h2>
            </div>
            <div className="flex gap-2">
              ...
              {franjas.length === 0 && (
                <button
                  ...
                  className="p-3 bg-gray-900 text-white rounded-2xl hover:bg-orange-600 transition-all shadow-lg"
                >
                  <Plus size={20} />
                </button>
              )}
          ...
          <div className="grid gap-3">
            {franjas.length === 0 ? (
              ...
              <div className="p-6 bg-gray-50/50 rounded-3xl border border-gray-100 group hover:bg-white hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-[10px] font-black text-orange-600 bg-orange-50 px-2 py-1 rounded-lg uppercase tracking-wider">
                      Horario General
                    </span>
                    <p className="mt-2 font-black text-gray-800 text-3xl">
                      {franjas[0].horini} — {franjas[0].horfin}
                    </p>
                  </div>
                  <div className="flex gap-1">
                    <button 
                      onClick={() => setModalFranja({ open: true, data: franjas[0] })} 
                      className="text-blue-500 hover:bg-blue-50 p-3 rounded-xl transition-colors"
                    >
                      <Edit3 size={20} />
                    </button>
                    <button 
                      onClick={() => handleDeleteFranja(franjas[0].id!)} 
        ...
        <section className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="p-3 bg-gray-900 text-white rounded-2xl hover:bg-blue-600 transition-all shadow-lg"
              >
                <Plus size={20} />
              </button>
            </div>
          ...
          <div className="grid gap-4">
            {promos.length === 0 ? (
              ...
              promos.map(p => (
                <div key={p.id} className="p-5 border border-gray-100 rounded-3xl bg-white hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300 group">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                         <button onClick={() => setModalPromo({ open: true, data: p })} className="text-blue-500 p-2 hover:bg-blue-50 rounded-lg"><Edit3 size={16} /></button>
                         <button onClick={() => handleDeletePromo(p.id!)} className="text-red-400 p-2 hover:bg-red-50 rounded-lg"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  </div>
                  <div className="mt-4 flex items-center gap-4 text-[11px] font-bold text-gray-400 border-t border-gray-50 pt-4 uppercase">
