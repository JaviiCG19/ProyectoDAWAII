...
export default function ModalPapeleraEmpresa({
  isOpen,
  onClose,
  onRestored,
}: Props) {
  const [eliminados, setEliminados] = useState<Restaurante[]>([]);
  ...
  const fetchEliminados = async () => {
    setLoading(true);
    try {
      const data = await getEmpresasEliminadas();
      setEliminados(data);
    } catch (error) {
  ...
  const handleRestaurar = async (id: number) => {
    ...
    try {
      ...
      setEliminados((prev) => prev.filter((e) => e.id !== id));
      onRestored();
    } catch (error) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Recupera los restaurantes eliminados
              </p>
            </div>
          </div>
